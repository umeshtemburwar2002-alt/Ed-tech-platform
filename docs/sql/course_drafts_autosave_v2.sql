-- ============================================================================
-- Persistent draft course autosave schema (v2)
-- Safe to apply to existing schemas that already have `course_sections`
-- and `course_lessons` tables.
-- ============================================================================
...
-- One active/in-progress draft per instructor (prevents duplicates)
...
-- ----------------------------------------------------------------------------
-- 2) Draft child tables (name-safe: does NOT conflict with course_sections)
-- ----------------------------------------------------------------------------
create table if not exists public.course_draft_sections (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists idx_course_draft_sections_draft_order
  on public.course_draft_sections (draft_id, order_index);

create table if not exists public.course_draft_lessons (
  id uuid primary key default gen_random_uuid(),
  section_id uuid not null references public.course_draft_sections(id) on delete cascade,
  title text not null,
  description text,
  video_url text,
  time_duration text,
...
create index if not exists idx_course_draft_lessons_section_order
  on public.course_draft_lessons (section_id, order_index);
...
drop trigger if exists trg_course_draft_sections_updated_at on public.course_draft_sections;
create trigger trg_course_draft_sections_updated_at
before update on public.course_draft_sections
for each row execute function public.set_updated_at();

drop trigger if exists trg_course_draft_lessons_updated_at on public.course_draft_lessons;
create trigger trg_course_draft_lessons_updated_at
before update on public.course_draft_lessons
for each row execute function public.set_updated_at();

-- ----------------------------------------------------------------------------
-- 4) RLS policies (instructor manages their own drafts)
-- ----------------------------------------------------------------------------
alter table public.course_drafts enable row level security;
alter table public.course_draft_sections enable row level security;
alter table public.course_draft_lessons enable row level security;
...
drop policy if exists "instructors_manage_own_course_draft_sections" on public.course_draft_sections;
create policy "instructors_manage_own_course_draft_sections"
on public.course_draft_sections
for all to authenticated
using (
  exists (
    select 1
    from public.course_drafts d
    where d.id = course_draft_sections.draft_id
...
drop policy if exists "instructors_manage_own_course_draft_lessons" on public.course_draft_lessons;
create policy "instructors_manage_own_course_draft_lessons"
on public.course_draft_lessons
for all to authenticated
using (
  exists (
    select 1
    ...
    where s.id = course_draft_lessons.section_id
      and d.instructor_id = auth.uid()
  )
)
with check (
...
begin
  ...
  -- Lock existing active/in_progress draft row for this instructor (if present).
  select *
  into v_draft
  from public.course_drafts d
  ...
  -- Completion scoring for UX/state
  v_completion := (
  ...
  -- Replace normalized sections + lessons (atomic in this transaction)
  delete from public.course_draft_lessons
  using public.course_draft_sections s
  where s.id = course_draft_lessons.section_id
    and s.draft_id = v_draft.id;

  delete from public.course_draft_sections where draft_id = v_draft.id;
  ...
  loop
    ...
    insert into public.course_draft_sections (draft_id, title, order_index)
    values (v_draft.id, coalesce(sec ->> 'title', 'Untitled Section'), sec_idx - 1)
...
-- ----------------------------------------------------------------------------
-- 6) Grants (for authenticated PostgREST access)
-- ----------------------------------------------------------------------------
grant select, insert, update, delete on table public.course_drafts to authenticated;
grant select, insert, update, delete on table public.course_draft_sections to authenticated;
grant select, insert, update, delete on table public.course_draft_lessons to authenticated;
